...
export default function ManagerApprovals() {
  ...
  const [viewCategory, setViewCategory] = useState<"sheets" | "shared">(
    "sheets",
  );
  ...
  const handleApproveSheet = async (sheetId: string) => {
    ...
    if (!res?.error) {
      setAllSheets((prev) =>
        prev.map((s) =>
          s.id === sheetId ? { ...s, submission_status: "approved" } : s,
        ),
      );
  ...
  const handleApproveShared = async (assignmentId: string) => {
    ...
    if (!res?.error) {
      setAllShared((prev) =>
        prev.map((s) =>
          s.id === assignmentId ? { ...s, status: "approved" } : s,
        ),
      );
  ...
  const handleRejectShared = async (assignmentId: string) => {
    ...
    if (!res?.error) {
      setAllShared((prev) =>
        prev.map((s) =>
          s.id === assignmentId
            ? { ...s, status: "rejected", rejection_reason: rejectComment }
            : s,
        ),
      );
      setRejectingId(null);
  ...
  const pendingSheets = allSheets.filter(
    (s) => s.submission_status === "submitted",
  );
  const reviewedSheets = allSheets.filter(
    (s) =>
      s.submission_status === "approved" || s.submission_status === "rejected",
  );
  const displayedSheets =
    activeTab === "pending" ? pendingSheets : reviewedSheets;

  const pendingShared = allShared.filter((s) => s.status === "submitted");
  const reviewedShared = allShared.filter(
    (s) => s.status === "approved" || s.status === "rejected",
  );
  const displayedShared =
    activeTab === "pending" ? pendingShared : reviewedShared;
  ...
  return (
    <div className="flex h-screen bg-[#F8F9FC] w-full overflow-hidden relative">
      <aside
        className={`fixed md:static bg-[#0F1729] w-[230px] h-screen z-50 transition-transform duration-200 shrink-0 ${mobile ? "translate-x-0" : "-translate-x-full"} md:translate-x-0 flex flex-col`}
      >
        ...
        <nav className="space-y-2 px-3 flex-1 mt-4">
          ...
          <Item
            ...
            active
            badge={
              pendingSheets.length + pendingShared.length > 0
                ? pendingSheets.length + pendingShared.length
                : undefined
            }
          />
          <Item
          ...
          <div className="flex flex-col gap-6 max-w-7xl mx-auto w-full">
            <div className="flex flex-col gap-1 min-w-0">
              ...
              <p className="text-neutral-500 text-sm leading-5 truncate">
                Review, approve, or decline employee goals and progress
                check-ins.
              </p>
            </div>
            ...
            <div className="border-neutral-200 border-b flex items-center gap-6 overflow-x-auto no-scrollbar w-full mt-2">
              <button
                ...
                className={`text-sm leading-5 pb-3 px-1 transition-colors whitespace-nowrap flex items-center gap-2 ${activeTab === "pending" ? "text-indigo-600 font-medium border-b-2 border-indigo-600" : "text-neutral-500 hover:text-neutral-900"}`}
              >
                <Clock className="size-4" /> Pending Review
              </button>
              <button
                onClick={() => setActiveTab("reviewed")}
                className={`text-sm leading-5 pb-3 px-1 transition-colors whitespace-nowrap flex items-center gap-2 ${activeTab === "reviewed" ? "text-indigo-600 font-medium border-b-2 border-indigo-600" : "text-neutral-500 hover:text-neutral-900"}`}
              >
                ...
                <table className="text-sm leading-5 w-full min-w-[800px]">
                  <thead>
                    <tr className="bg-neutral-50 border-b border-neutral-200/60 text-neutral-500">
                      <th className="font-medium text-left px-5 py-3 whitespace-nowrap">
                        Employee
                      </th>
                      {viewCategory === "shared" && (
                        <th className="font-medium text-left px-5 py-3 whitespace-nowrap">
                      ...
                      <th className="font-medium text-right px-5 py-3">
                        Actions / Notes
                      </th>
                    </tr>
                  </thead>
                        ...
                        displayedSheets.map((sheet, idx) => (
                          ...
                          >
                            ...
                            <td className="px-5 py-4 whitespace-nowrap">
                              {sheet.submission_status === "submitted" && (
                              ...
                              {sheet.submission_status === "rejected" && (
                                ...
                                {activeTab === "pending" ? (
                                  <>
                                    {rejectingId === sheet.id ? (
                                      ...
                                      <>
                                        <Button
                                          onClick={() =>
                                            setRejectingId(sheet.id)
                                          }
                                          variant="outline"
                                          size="sm"
                                          className="h-8 text-rose-600 border-rose-200 hover:bg-rose-50 bg-white"
                                        >
                                          <MessageSquareX className="size-3.5 mr-1" />{" "}
                                          Decline
                                        </Button>
                                        <Button
                                          onClick={() =>
                                            handleApproveSheet(sheet.id)
                                          }
                                          size="sm"
                                  ...
                                  <div className="flex justify-end">
                                    ...
                                    sheet.rejection_reason ? (
                                      <span className="text-xs text-neutral-500 italic max-w-xs text-right truncate">
                      ...
                      (displayedShared.length === 0 ? (
                        <tr>
                          ...
                          >
                            <CheckCircle className="size-8 text-neutral-300 mx-auto mb-3" />
                            {activeTab === "pending"
                              ? "No pending shared goal submissions."
                              : "No reviewed shared goals found."}
                          </td>
                        </tr>
                      ) : (
                        displayedShared.map((sg, idx) => (
                          ...
                          >
                            <td className="px-5 py-4 font-medium text-neutral-900 whitespace-nowrap">
                              {sg.employee?.full_name ||
                                "Employee ID: " + sg.employee_id}
                            </td>
                            <td className="px-5 py-4 whitespace-nowrap">
                              <div className="font-semibold text-neutral-900 max-w-[200px] truncate">
                                {sg.shared_goals?.title}
                              </div>
                              <div className="text-xs text-neutral-500">
                                Target: {sg.shared_goals?.target_value}
                              </div>
                            </td>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-3xl max-h-[85vh] overflow-hidden flex flex-col">
              ...
              <div>
                ...
                <p className="text-sm text-neutral-500 mt-0.5">
                  ...
                  <span className="font-medium text-neutral-900">
                    {sheetDetails.profile?.full_name}
                  </span>
                  {sheetDetails.profile?.department
                    ? ` • ${sheetDetails.profile.department}`
                    : ""}
                </p>
              </div>
            ...
            <div className="overflow-y-auto p-5 space-y-4 custom-scrollbar bg-white">
              {sheetDetails.goals?.map((goal: any, index: number) => (
                <div
                  key={goal.id}
                  className="p-4 border border-neutral-200/80 rounded-xl bg-white shadow-sm"
                >
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <Badge
                      variant="outline"
                      className="text-neutral-500 border-neutral-200 shrink-0 font-medium bg-neutral-50"
                    >
                      {goal.thrust_area}
                    </Badge>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-3 mt-4 pt-4 border-t border-neutral-100 text-[13px] font-medium text-neutral-700">
                    <div className="flex flex-col bg-neutral-50 px-3 py-1.5 rounded-lg border border-neutral-100 min-w-[100px]">
                      <span className="text-[10px] uppercase tracking-wider text-neutral-400 font-bold mb-0.5">
                        Weightage
                      </span>
                    ...
                    <div className="flex flex-col bg-neutral-50 px-3 py-1.5 rounded-lg border border-neutral-100 min-w-[100px]">
                      ...
                      <span>
                        {goal.target_value} {goal.uom_type}
                      </span>
                    </div>
                    <div className="flex flex-col bg-neutral-50 px-3 py-1.5 rounded-lg border border-neutral-100 min-w-[100px]">
                      <span className="text-[10px] uppercase tracking-wider text-neutral-400 font-bold mb-0.5">
            ...
            <div className="p-5 border-t border-neutral-100 bg-neutral-50/50 flex justify-end">
              <Button
                onClick={closeDetailsModal}
                className="bg-white text-neutral-700 border border-neutral-200 hover:bg-neutral-50"
              >
                Close Details
              </Button>
            </div>
...
function Item({ icon: Icon, href, label, active, badge }: any) {
  return (
    <Link
      href={href}
      className={`flex items-center justify-between px-4 py-3 rounded-xl transition-colors ${active ? "bg-indigo-600 text-white font-medium" : "text-white/70 hover:bg-white/10 hover:text-white"}`}
    >
