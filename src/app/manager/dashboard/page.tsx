...
export default function ManagerDashboard() {
  ...
  async function loadData() {
    try {
      ...
      const progressArray = Object.values(employeeMap)
        .map((emp) => ({
          name: emp.name,
  ...
  const handleReject = async (sheetId: string) => {
    if (!confirm("Are you sure you want to return this goal sheet for rework?"))
      return;
    const res = await rejectGoalSheet(
      sheetId,
      "Returned for rework from dashboard",
    );
    if (!res.error) {
  ...
  const overallDeptProgress =
    teamSize > 0
      ? Math.round(
          teamProgress.reduce((acc, curr) => acc + curr.progress, 0) / teamSize,
        )
      : 0;
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <Card className="shadow-sm rounded-2xl border-neutral-200/60 p-5">
                ...
                <CardContent className="p-0 mt-3 flex flex-col gap-1">
                  <div className="font-bold text-neutral-950 text-2xl">
                    {teamSize}
                  </div>
                  <span className="text-neutral-500 text-xs">
                    Direct reports
                    ...
                    {pendingSheets.length === 0 ? (
                      <div className="text-center py-10 px-4 rounded-xl border border-dashed border-neutral-200 bg-neutral-50">
                        ...
                        <p className="text-neutral-500 text-sm">
                          No goal sheets are pending approval.
                        </p>
                      </div>
                    ) : (
                      pendingSheets.map((sheet: any) => {
                        const isExpanded = expandedSheet === sheet.id;
                        const sheetGoals = allGoals.filter(
                          (g) => g.goal_sheet_id === sheet.id,
                        );

                        return (
                          <div
                            key={sheet.id}
                            className={`rounded-xl border border-solid transition-colors ${isExpanded ? "border-indigo-200 bg-indigo-50/10" : "border-neutral-200 bg-white"} flex p-4 flex-col gap-4`}
                          >
                              ...
                              <div className="flex flex-col gap-4 mt-2">
                                <div className="rounded-lg bg-white border border-neutral-200 overflow-hidden">
                                  ...
                                  {sheetGoals.length === 0 ? (
                                    ...
                                    sheetGoals.map((g: any) => (
                                      <div
                                        key={g.id}
                                        className="grid grid-cols-[1fr_90px_70px] md:grid-cols-[1fr_90px_70px_70px] text-xs border-t border-neutral-200 px-4 py-3 items-center"
                                      >
                                        <span
                                          className="text-neutral-900 font-medium truncate pr-2"
                                          title={g.title}
                                        >
                                          {g.title}
                                        </span>
                                ...
                                <div className="rounded-lg bg-amber-50 flex px-3 py-2 items-center gap-2 border border-amber-100">
                                  ...
                                  <span className="text-amber-700 text-xs">
                                    Approving this sheet will lock the goals for
                                    the employee.
                                  </span>
                                </div>
                                ...
                                <div className="flex items-center gap-2 pt-2">
                                  ...
                                  >
                                    <RotateCcw className="size-4 mr-1" /> Return
                                    for Rework
                                  </Button>
                                </div>
                              </div>
              ...
              <div className="flex flex-col gap-6">
                ...
                <Card className="shadow-sm rounded-2xl border-neutral-200/60 p-6 flex flex-col gap-4">
                  <CardHeader className="p-0 gap-1">
                    ...
                    <CardDescription className="text-sm">
                      Overall goal completion by member
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="flex p-0 flex-col gap-4 mt-2">
                    {teamProgress.length === 0 ? (
                      <p className="text-sm text-neutral-500">
                        No active goals to track yet.
                      </p>
                    ) : (
