...
export default function ManagerCheckinsPage() {
  ...
  useEffect(() => {
    ...
    filtered.forEach((g: any) => {
      ...
      const status = (g.goal_status || "on_track")
        .toLowerCase()
        .replace(" ", "_");
      if (status === "completed") employeeStats[empName].completed += 1;
      else if (status === "behind" || status === "at_risk")
        employeeStats[empName].behind += 1;
  ...
  const handleExport = (format: "csv" | "excel") => {
    ...
    const rows = filteredGoals.map((g) => [
      ...
      escapeCSV(g.title),
      escapeCSV(g.target_value),
      escapeCSV(g.actual_achievement || 0),
    ...
    link.setAttribute(
      "download",
      `Team_Checkins_${selectedQuarter}_${new Date().toISOString().split("T")[0]}.csv`,
    );
  ...
  const handleSaveComment = async (goalId: string) => {
    ...
    if (!error) {
      setAllGoals(
        allGoals.map((g) =>
          g.id === goalId ? { ...g, manager_comment: text } : g,
        ),
      );
  ...
  return (
    <div className="flex h-screen bg-[#F8F9FC]">
      <aside
        className={`fixed md:static bg-[#0F1729] w-[230px] h-screen z-50 transition-transform ${mobile ? "translate-x-0" : "-translate-x-full"} md:translate-x-0 flex flex-col`}
      >
        <div className="p-5 flex items-center gap-3">
          ...
          <div className="flex flex-col text-white">
            <span className="font-bold text-sm">OMP</span>
          </div>
        </div>
            ...
            <div className="flex justify-between items-end gap-4">
              <div>
                ...
                <p className="text-neutral-500 text-sm">
                  Review quarterly goal progress.
                </p>
              </div>
                ...
                <div className="flex bg-neutral-100 rounded-lg p-1">
                  {["All", "Q1", "Q2", "Q3", "Q4"].map((q) => (
                    <button
                      ...
                      className={`px-4 py-1.5 rounded-md text-sm font-medium ${selectedQuarter === q ? "bg-white text-indigo-600 shadow-sm" : "text-neutral-500"}`}
                    >
                      {q}
                    </button>
                  ))}
                  ...
                  <tbody>
                    {filteredGoals.length === 0 ? (
                      <tr>
                        ...
                        >
                          No active goals found for {selectedQuarter}.
                        </td>
                      </tr>
                    ) : (
                      filteredGoals.map((g) => (
                        ...
                        >
                          ...
                          <td className="p-4 text-neutral-600 truncate max-w-[200px]">
                            {g.title}
                          </td>
                            ...
                            {editingCommentId === g.id ? (
                              ...
                              <div className="flex items-center justify-between">
                                <span className="text-xs max-w-[200px] truncate">
                                  {g.manager_comment || "None"}
                                </span>
                                <button
                                  onClick={() => {
                                    setEditingCommentId(g.id);
                                    setCommentInputs({
                                      ...commentInputs,
                                      [g.id]: g.manager_comment || "",
                                    });
                                  }}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-2">
              <Card className="shadow-sm rounded-2xl p-6">
                <CardHeader className="p-0 mb-6">
                  <CardTitle className="text-lg">Quarterly Trend</CardTitle>
                </CardHeader>
                <CardContent className="p-0">
                  <ChartContainer
                    className="h-[250px] w-full"
                    config={{ averageProgress: { color: "#4f46e5" } }}
                  >
                    <LineChart data={trendData}>
                      <XAxis dataKey="quarter" />
                      <YAxis domain={[0, 100]} />
                      <Line dataKey="averageProgress" stroke="#4f46e5" />
                    </LineChart>
                  </ChartContainer>
                </CardContent>
              </Card>
                ...
                <CardContent className="p-0">
                  ...
                  >
                    <BarChart data={breakdownData}>
                      ...
                      <Bar dataKey="behind" stackId="a" fill="#f43f5e" />
                    </BarChart>
                  </ChartContainer>
                </CardContent>
              </Card>
...
function Item({ icon: Icon, href, label, active, badge }: any) {
  return (
    <Link
      href={href}
      className={`flex items-center justify-between px-4 py-3 rounded-xl transition-colors ${active ? "bg-indigo-600 text-white font-medium shadow-sm" : "text-white/70 hover:bg-white/10"}`}
    >
      <div className="flex items-center gap-3">
        <Icon size={18} /> {label}
      </div>
