...
export default function TeamReportsPage() {
  ...
  async function loadData() {
    ...
    try {
      ...
      // 1. Calculate Breakdown Chart Data (Per-Employee Goal Status)
      ...
      // 2. Calculate Trend Data (Average progress by Quarter)
  ...
  const handleExport = (format: "csv" | "excel") => {
    ...
    // For both CSV and Excel, a standard UTF-8 CSV works perfectly natively.
    // If you explicitly need an .xlsx extension, you would need to install `xlsx` (npm install xlsx).
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    
    const dateStr = new Date().toISOString().split('T')[0];
    const extension = format === "excel" ? "csv" : "csv"; // Use CSV for both without external libs
    
    link.setAttribute("href", url);
    link.setAttribute("download", `Team_Checkins_${dateStr}.${extension}`);
  ...
  const handleSaveComment = async (goalId: string) => {
    ...
    if (!error) {
      setGoals(goals.map(g => g.id === goalId ? { ...g, manager_comment: text } : g));
    ...
    <div className="flex h-screen bg-[#F8F9FC]">
      ...
      >
        ...
        <nav className="space-y-2 px-3 flex-1 mt-4">
          ...
          <Item icon={BarChart3} href="/manager/reports" label="Team Reports" active />
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <main className="p-4 md:p-8 overflow-auto flex-1">
          <div className="max-w-7xl mx-auto flex flex-col gap-6">
            ...
            {/* Table Card */}
            <Card className="shadow-sm rounded-2xl border-neutral-200/60 overflow-hidden">
                    ...
                    {goals.length === 0 ? (
                      <tr>
                        <td colSpan={8} className="p-8 text-center text-neutral-500">
                          No active goals found for your team.
                        </td>
                      </tr>
                    ) : (
                      goals.map((g) => {
              ...
              <div className="flex justify-between items-center p-4 border-t border-neutral-100 bg-neutral-50/30">
                <div className="text-neutral-500 text-sm">
                  Showing <span className="font-medium text-neutral-900">1–{goals.length}</span> of <span className="font-medium text-neutral-900">{goals.length}</span>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-2">
              ...
              <Card className="shadow-sm rounded-2xl border-neutral-200/60 p-6 flex flex-col">
                <CardHeader className="p-0 mb-6">
                  ...
                  <CardDescription className="text-neutral-500 mt-1">
                    Average goal completion % across all department goals
                  </CardDescription>
                </CardHeader>
              ...
              <Card className="shadow-sm rounded-2xl border-neutral-200/60 p-6 flex flex-col">
                <CardHeader className="p-0 mb-6">
                  ...
                  <CardDescription className="text-neutral-500 mt-1">
                    Per-employee goal distribution by current status
                  </CardDescription>
                </CardHeader>
                <CardContent className="p-0 flex-1">
