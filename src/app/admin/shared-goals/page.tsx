...
export default function SharedGoalsManagement() {
  ...
  }, [searchQuery, typeFilter, sharedGoals]);
  ...
  async function handleCreateGoal(e: React.FormEvent) {
    ...
    try {
      ...
      if (result.error) {
        ...
        setFormData({ title: "", description: "", target: "", uom: "", type: "all", department: "", selectedEmployeeIds: [], weightage: 10 });
        setModalDeptFilter("");
        loadData();
      }
    } catch (err) {
            ...
            <form onSubmit={handleCreateGoal} className="overflow-y-auto p-6 flex flex-col gap-5">
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1.5">Unit of Measure (UOM) *</label>
                    <input required type="text" value={formData.uom} onChange={(e) => setFormData({...formData, uom: e.target.value})} className="w-full px-4 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" placeholder="e.g., USD, %, Units" />
                  </div>
                </div>
                <div>
                ...
                {formData.type === "department" && (
                  <div className="animate-in fade-in slide-in-from-top-2">
                    ...
                    <select required value={formData.department} onChange={(e) => setFormData({...formData, department: e.target.value})} className="w-full px-4 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 bg-white">
                      ...
                      {availableDepartments.map((dept: any) => (<option key={dept} value={dept}>{dept}</option>))}
                    </select>
                  </div>
                )}
                  ...
                  <div className="animate-in fade-in slide-in-from-top-2 border border-slate-200 rounded-lg p-4 bg-slate-50 flex flex-col gap-4">
                    ...
                    <div>
                      ...
                      <div className="max-h-48 overflow-y-auto space-y-1.5 bg-white border border-slate-200 rounded-lg p-2">
                        {employees.filter(emp => !modalDeptFilter || emp.department === modalDeptFilter).map(emp => (
                            <label key={emp.id} className="flex items-center gap-3 p-2 hover:bg-slate-50 rounded-md cursor-pointer border border-transparent hover:border-slate-100 transition-colors">
        ...
        <div className="p-6 md:p-8 flex flex-col gap-8 max-w-7xl mx-auto w-full">
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
            <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-white">
              ...
              <span className="text-sm text-slate-500">Showing 1–{Math.min(8, filteredGoals.length)} of {filteredGoals.length}</span>
            </div>
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredGoals.length === 0 ? (
                    <tr><td colSpan={6} className="px-6 py-12 text-center text-slate-500">No shared goals found. Create one to get started!</td></tr>
                  ) : (
                    filteredGoals.slice(0, 8).map((goal) => {
                      const isCompleted = goal.progress >= 100;
                      const isOnTrack = goal.progress > 0 && goal.progress < 100;
                        ...
                        <tr key={goal.id} className="hover:bg-slate-50/50 transition-colors group">
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex flex-col">
                              ...
                              <span className="text-xs text-slate-500 truncate max-w-[200px]">{goal.description || "No description provided"}</span>
                            </div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className="text-sm text-slate-600">{formatDate(goal.created_at)}</span>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex flex-col gap-1.5">
                              <div className="flex items-center justify-between">
                                <span className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${isCompleted ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' : isOnTrack ? 'bg-blue-50 text-blue-700 border border-blue-100' : 'bg-slate-50 text-slate-500 border border-slate-200'}`}>
                                  {isCompleted ? 'Completed' : isOnTrack ? 'On Track' : 'Not Started'}
                                </span>
                                <span className="text-xs font-semibold text-slate-600">{goal.progress || 0}%</span>
                              </div>
                              <div className="h-1.5 w-32 bg-slate-100 rounded-full overflow-hidden">
                                <div className={`h-full rounded-full transition-all duration-500 ${getProgressColor(goal.progress || 0)}`} style={{ width: `${Math.max(0, Math.min(100, goal.progress || 0))}%` }} />
                              </div>
                            </div>
                          </td>
                        </tr>
            ...
            <div className="p-4 border-t border-slate-100 flex items-center justify-between bg-white">
              <span className="text-sm text-slate-500 hidden sm:block">Showing 1–{Math.min(8, filteredGoals.length)} of {filteredGoals.length} goals</span>
