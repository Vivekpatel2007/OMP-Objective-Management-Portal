...
export default function SharedGoalsManagement() {
  ...
  async function loadData() {
    try {
      setLoading(true);
      const [userProfile, goalsResponse, employeesRes, reportRes] =
        await Promise.all([
          getCurrentUserProfile(),
          getSharedGoals(),
          getEmployees(),
      ...
      const { data: goalsData } = goalsResponse?.data
        ? goalsResponse
        : await goalsResponse;
  ...
  useEffect(() => {
    ...
    if (typeFilter !== "All Types") {
      result = result.filter(
        (g) => g.assignment_type?.toLowerCase() === typeFilter.toLowerCase(),
      );
    }
      ...
      setSharedGoals((prev) =>
        prev.map((g) => {
          if (g.id === goalId) {
            return {
              ...g,
              shared_goal_assignments: (g.shared_goal_assignments || []).map(
                (a: any) =>
  ...
  async function handleCreateGoal(e: React.FormEvent) {
    ...
    try {
      const payload = {
        ...
        department:
          formData.type === "department" ? formData.department : undefined,
        employees:
          formData.type === "employee"
            ? employees.filter((emp) =>
                formData.selectedEmployeeIds.includes(emp.id),
              )
            : [],
  ...
  const allOrgGoals = sharedGoals.filter(
    (g) => g.assignment_type === "all",
  ).length;
  const deptGoals = sharedGoals.filter(
    (g) => g.assignment_type === "department",
  ).length;
  ...
  return (
    <div className="min-h-screen bg-[#f4f7fb] text-slate-900 flex w-full font-sans relative">
      <aside
        className={`shrink-0 min-h-screen bg-[#111115] text-white p-6 flex-col gap-8 w-64 fixed md:sticky top-0 z-40 transition-transform duration-300 ease-in-out ${mobileMenuOpen ? "translate-x-0 flex" : "-translate-x-full md:translate-x-0 md:flex hidden"}`}
      >
        ...
        <nav className="flex flex-col gap-1.5 mt-4">
          ...
          <Link
            ...
            className="rounded-lg text-slate-400 hover:bg-white/5 hover:text-white text-sm font-medium flex px-4 py-3 items-center gap-3 transition-colors"
          >
            <Settings2 className="size-4" /> Goal Sheets
          </Link>
        </nav>
            ...
            <div className="flex items-center justify-between p-5 border-b border-slate-100">
              <div>
                ...
                <p className="text-sm text-slate-500">
                  Distribute a unified goal across the organization, department,
            ...
            >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-4 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 bg-white"
                    >
                ...
                {formData.type === "department" && (
                  <div className="animate-in fade-in slide-in-from-top-2">
                    ...
                    >
                      ...
                      {availableDepartments.map((dept: any, idx) => (
                  ...
                  <div className="animate-in fade-in slide-in-from-top-2 border border-slate-200 rounded-lg p-4 bg-slate-50 flex flex-col gap-4">
                    <div>
                      ...
                      <select
                        ...
                        className="w-full px-3 py-1.5 border border-slate-200 rounded-md text-sm bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
                      >
                        <option value="">All Departments</option>
                        {availableDepartments.map((dept: any, idx) => (
                          <option key={`filter-dept-${idx}`} value={dept}>
                            {dept}
                          </option>
                        ))}
                      </select>
                      ...
                      <div className="max-h-48 overflow-y-auto space-y-1.5 bg-white border border-slate-200 rounded-lg p-2">
                        {employees
                          .filter(
                            (emp) =>
                              !modalDeptFilter ||
                              emp.department === modalDeptFilter,
                          )
                          .map((emp, idx) => (
                            <label
                              key={`emp-${emp.id || idx}`}
                              className="flex items-center gap-3 p-2 hover:bg-slate-50 rounded-md cursor-pointer border border-transparent hover:border-slate-100 transition-colors"
                            >
                              <input
                                type="checkbox"
                                className="rounded text-blue-600 focus:ring-blue-500"
                                checked={formData.selectedEmployeeIds.includes(
                                  emp.id,
                                )}
                                onChange={(e) => {
                                  ...
                                  setFormData((prev) => ({
                                    ...
                                    selectedEmployeeIds: checked
                                      ? [...prev.selectedEmployeeIds, emp.id]
              ...
              <div className="flex items-center gap-3 pt-4 border-t border-slate-100 mt-2">
                <button
                  type="button"
                  onClick={() => setIsCreateModalOpen(false)}
                  className="px-4 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-100 rounded-lg transition-colors ml-auto"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="px-6 py-2 text-sm font-semibold text-white bg-blue-600 hover:bg-blue-700 rounded-lg shadow-sm transition-colors disabled:opacity-50 flex items-center gap-2"
                >
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col gap-1">
                ...
                <p className="text-slate-500 text-sm">
                  Create and monitor overarching goals distributed across teams
                </p>
              </div>
            </div>
            <button
              onClick={() => setIsCreateModalOpen(true)}
              className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2.5 rounded-lg text-sm font-semibold shadow-sm transition-colors flex items-center gap-2 w-full md:w-auto justify-center"
            >
          ...
          <div className="flex flex-col md:flex-row gap-4 w-full">
            <div className="relative flex-1 max-w-md">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2.5 bg-white border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-shadow"
              />
            </div>
            <div className="relative w-full md:w-48">
              <select
                ...
                className="w-full appearance-none pl-4 pr-10 py-2.5 bg-white border border-slate-200 rounded-lg text-sm font-medium text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-shadow"
              >
            ...
            <div className="relative w-full md:w-40">
              <select
                ...
                className="w-full appearance-none pl-10 pr-10 py-2.5 bg-white border border-slate-200 rounded-lg text-sm font-medium text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-shadow"
              >
                <option value="All Quarters">All Quarters</option>
                <option value="Q1">Q1</option>
                <option value="Q2">Q2</option>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-sm flex items-center gap-5">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-3xl font-bold font-serif text-purple-600">
                  {allOrgGoals}
                </span>
              </div>
            </div>
            <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-sm flex items-center gap-5">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-3xl font-bold font-serif text-emerald-600">
                  {deptGoals}
                </span>
              </div>
            </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
            <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-white">
              ...
              <span className="text-sm text-slate-500">
                Showing 1–{Math.min(10, filteredGoals.length)} of{" "}
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredGoals.length === 0 ? (
                    <tr>
                      ...
                      >
                        No shared goals found matching the filters.
                      </td>
                    </tr>
                  ) : (
                          ...
                          <tr className="hover:bg-slate-50/50 transition-colors group">
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="flex flex-col">
                                ...
                                <span className="text-xs text-slate-500 truncate max-w-[200px] mt-0.5">
                                  {goal.description ||
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                              <span
                                className={`inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border ${getTypeStyles(goal.assignment_type)}`}
                              >
                                {goal.assignment_type === "all"
                                  ? "Organization"
                                  : goal.assignment_type}
                                {goal.assignment_type === "department" &&
                                  goal.department &&
                                  ` : ${goal.department}`}
                              </span>
                            </td>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="flex flex-wrap gap-1.5 max-w-[200px]">
                                <Badge
                                  variant="outline"
                                  className="bg-slate-50 text-slate-600 border-slate-200 text-[10px] font-semibold"
                                >
                                  {totalCount} Assigned
                                </Badge>
                                {submittedCount > 0 && (
                                    ...
                                    {assignments.filter(
                                      ...
                                      assignments
                                        ...
                                        .map((a: any, aIdx: number) => (
                                          <div
                                            key={`assignment-${a.id || aIdx}`}
                                            className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-3 border border-slate-100 rounded-lg bg-slate-50/80"
                                          >
                                              ...
                                              {rejectingId === a.id ? (
                                                <div className="flex items-center gap-2">
                                                  ...
                                                  <Button
                                                    ...
                                                    className="bg-rose-600 hover:bg-rose-700 h-7 text-xs text-white px-3"
                                                  >
                                                    Confirm
                                                  </Button>
                                                  <Button
                                                ...
                                                <>
                                                  <Button
                                                    ...
                                                    className="text-rose-600 border-rose-200 h-7 text-xs hover:bg-rose-50 px-3"
                                                  >
                                                    Decline
                                                  </Button>
                                                  <Button
                                                    ...
                                                    className="bg-emerald-600 hover:bg-emerald-700 h-7 text-xs text-white px-3"
                                                  >
            ...
            <div className="p-4 border-t border-slate-100 flex items-center justify-between bg-white">
              ...
              <div className="flex items-center gap-1 ml-auto sm:ml-0">
                <button className="px-3 py-1.5 border border-slate-200 rounded-md text-sm font-medium text-slate-600 hover:bg-slate-50 transition-colors disabled:opacity-50">
                  Previous
                </button>
                <button className="size-8 rounded-md bg-blue-600 text-white text-sm font-medium shadow-sm">
                  1
                </button>
                <button className="px-3 py-1.5 border border-slate-200 rounded-md text-sm font-medium text-slate-600 hover:bg-slate-50 transition-colors">
                  Next
                </button>
              </div>
            </div>
