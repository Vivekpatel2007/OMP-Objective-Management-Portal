...
import { getAdminDashboard, getCycles, activateCycle, getEmployees } from "@/services/adminservice";
...
export default function AdminDashboard() {
  ...
  // Data States
  const [dashboardStats, setDashboardStats] = useState<any>({ employees: 0, activeCycle: null });
  ...
  async function loadData() {
    try {
      ...
      const [userRes, adminRes, cyclesRes, reportRes, employeesRes] = await Promise.all([
      ...
      if (cyclesRes?.data) {
        ...
        const sortedCycles = cyclesRes.data.sort((a, b) => (b.is_active ? 1 : 0) - (a.is_active ? 1 : 0));
      ...
      if (reportRes) {
        ...
        const totalEligibleEmployees = employees.filter((e: any) => e.role !== "admin").length;
        const employeesWithSheets = new Set(sheets.map((s: any) => s.employee_id)).size;
        const adoptionPercent = totalEligibleEmployees > 0 
        ...
        let totalProgress = 0;
        const deptMap: Record<string, { total: number; completed: number; onTrack: number; notStarted: number }> = {};

        // Initialize departments
        employees.forEach((emp: any) => {
          if (emp.department && emp.role !== "admin") {
            if (!deptMap[emp.department]) {
              deptMap[emp.department] = { total: 0, completed: 0, onTrack: 0, notStarted: 0 };
            }
          }
        });
        ...
        setOrgCompletion(goals.length > 0 ? Math.round(totalProgress / goals.length) : 0);

        // Format for UI
        const deptArray = Object.keys(deptMap).map(dept => {
          const stats = deptMap[dept];
          return {
            name: dept,
            total: stats.total,
            completedPct: stats.total > 0 ? (stats.completed / stats.total) * 100 : 0,
            onTrackPct: stats.total > 0 ? (stats.onTrack / stats.total) * 100 : 0,
            notStartedPct: stats.total > 0 ? (stats.notStarted / stats.total) * 100 : 0,
          };
        }).filter(d => d.total > 0);
  ...
  return (
    <div className="min-h-screen bg-[#f4f7fb] text-slate-900 flex w-full font-sans overflow-x-hidden">
      ...
      {/* SIDEBAR */}
      <aside className={`shrink-0 min-h-screen bg-[#111115] text-white p-6 flex-col gap-8 w-64 fixed md:sticky top-0 z-50 transition-transform duration-300 ease-in-out ${mobile ? "translate-x-0 flex" : "-translate-x-full md:translate-x-0 md:flex hidden"}`}>
        ...
        <nav className="flex flex-col gap-1.5 mt-4">
          <Link href="/admin/dashboard" className="rounded-lg bg-blue-600 text-white text-sm font-medium flex px-4 py-3 items-center gap-3 shadow-lg shadow-blue-600/20">
            <LayoutDashboard className="size-4" /> Dashboard
          </Link>
          <Link href="/admin/cycles" className="rounded-lg text-slate-400 hover:bg-white/5 hover:text-white text-sm font-medium flex px-4 py-3 items-center gap-3 transition-colors">
            <CalendarClock className="size-4" /> Goal Cycles
          </Link>
          <Link href="/admin/emp-management" className="rounded-lg text-slate-400 hover:bg-white/5 hover:text-white text-sm font-medium flex px-4 py-3 items-center gap-3 transition-colors">
            <Users className="size-4" /> Employee Management
          </Link>
          <Link href="/admin/manager-management" className="rounded-lg text-slate-400 hover:bg-white/5 hover:text-white text-sm font-medium flex px-4 py-3 items-center gap-3 transition-colors">
      ...
      <main className="flex p-6 md:p-8 flex-col flex-1 gap-8 overflow-auto w-full max-w-7xl mx-auto">
        <header className="flex justify-between items-center">
          <div className="flex gap-3 items-center">
            <button onClick={() => setMobile(!mobile)} className="md:hidden p-2 -ml-2 text-slate-500">
              {mobile ? <X className="size-6" /> : <Menu className="size-6" />}
            </button>
            <div className="flex flex-col gap-1">
              <h1 className="font-bold text-2xl text-slate-900 font-serif">Admin Dashboard</h1>
              <p className="text-slate-500 text-sm hidden md:block">
                Welcome back, {profile?.full_name || "System Admin"}. Here is your organization overview.
              </p>
            </div>
          </div>
        ...
        <section className="grid grid-cols-2 md:grid-cols-5 gap-4 md:gap-6">
          <div className="rounded-xl bg-white shadow-sm border border-slate-200 flex p-5 flex-col gap-3 justify-between">
            <div className="flex justify-between items-start gap-1">
              <span className="text-slate-500 font-bold text-xs tracking-wider uppercase">Total Employees</span>
              <div className="p-1.5 rounded-lg bg-blue-50 text-blue-600">
                <Users className="size-4" />
              </div>
            </div>
            <div className="font-bold text-3xl font-serif text-slate-900">{dashboardStats.employees}</div>
          </div>
          
          <div className="rounded-xl bg-white shadow-sm border border-slate-200 flex p-5 flex-col gap-3 justify-between">
            <div className="flex justify-between items-start gap-1">
              <span className="text-slate-500 font-bold text-xs tracking-wider uppercase">Active Cycle</span>
              <div className="p-1.5 rounded-lg bg-purple-50 text-purple-600">
                <CalendarClock className="size-4" />
              </div>
            </div>
            <div className="font-bold text-lg text-slate-900 line-clamp-1 mt-1" title={dashboardStats.activeCycle?.cycle_name}>
          ...
          <div className="rounded-xl bg-white shadow-sm border border-slate-200 flex p-5 flex-col gap-3 justify-between">
            <div className="flex justify-between items-start gap-1">
              <span className="text-slate-500 font-bold text-xs tracking-wider uppercase">Org Completion</span>
              <div className="p-1.5 rounded-lg bg-emerald-50 text-emerald-600">
                <TrendingUp className="size-4" />
              </div>
            </div>
            <div className="text-emerald-600 font-bold text-3xl font-serif">{orgCompletion}%</div>
          </div>
          
          <div className="rounded-xl bg-white shadow-sm border border-slate-200 flex p-5 flex-col gap-3 justify-between">
            <div className="flex justify-between items-start gap-1">
              <span className="text-slate-500 font-bold text-xs tracking-wider uppercase">Pending Approvals</span>
              <div className="p-1.5 rounded-lg bg-amber-50 text-amber-600">
                <Clock className="size-4" />
              </div>
            </div>
            <div className="text-amber-500 font-bold text-3xl font-serif">{pendingApprovals}</div>
          </div>
          
          <div className="rounded-xl bg-white shadow-sm border border-slate-200 flex p-5 flex-col gap-3 justify-between">
            <div className="flex justify-between items-start gap-1">
              <span className="text-slate-500 font-bold text-xs tracking-wider uppercase">Goal Adoption</span>
              <div className="p-1.5 rounded-lg bg-indigo-50 text-indigo-600">
                <Sparkles className="size-4" />
              </div>
            </div>
            <div className="text-indigo-600 font-bold text-3xl font-serif">{goalAdoption}%</div>
          </div>
        </section>
        ...
        <section className="flex flex-col xl:flex-row gap-8">
          ...
          <div className="flex flex-col gap-8 flex-1 min-w-0">
            ...
            <div className="rounded-xl bg-white shadow-sm border border-slate-200 flex flex-col overflow-hidden">
              <div className="p-5 border-b border-slate-100 flex flex-col md:flex-row justify-between md:items-center gap-4">
                <div className="flex flex-col">
                  <h2 className="font-bold text-lg text-slate-900">Goal Cycle Management</h2>
                  <p className="text-slate-500 text-sm">Manage organization goal cycles</p>
                </div>
                <Link href="/admin/cycles" className="inline-flex font-semibold rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition-colors text-sm px-4 py-2 items-center gap-2 shadow-sm">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse min-w-[500px]">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {cycles.length === 0 ? (
                      <tr>
                        <td colSpan={3} className="px-6 py-12 text-center text-slate-500">No cycles found.</td>
                      </tr>
                    ) : (
                      cycles.map((cycle) => (
                        <tr key={cycle.id} className="hover:bg-slate-50/50 transition-colors group">
            ...
            <div className="rounded-xl bg-white shadow-sm border border-slate-200 flex flex-col p-6 gap-6">
              <div className="flex flex-col md:flex-row justify-between md:items-end gap-4 border-b border-slate-100 pb-4">
                <div className="flex flex-col">
                  <h2 className="font-bold text-lg text-slate-900">Organization Completion Dashboard</h2>
                  <p className="text-slate-500 text-sm">Goal status aggregated by department</p>
                </div>
                <div className="flex items-center gap-4 text-xs font-semibold uppercase tracking-wider text-slate-500">
                  <span className="flex items-center gap-1.5"><span className="size-2.5 bg-slate-200 rounded-full" /> Not Started</span>
                  <span className="flex items-center gap-1.5"><span className="size-2.5 bg-amber-400 rounded-full" /> In Progress</span>
                  <span className="flex items-center gap-1.5"><span className="size-2.5 bg-emerald-500 rounded-full" /> Completed</span>
                </div>
              </div>
              
              <div className="flex flex-col gap-5">
                {deptProgress.length === 0 ? (
                  <p className="text-sm text-slate-500 py-4 text-center">No active departmental goal data available.</p>
                ) : (
                  deptProgress.map((dept, idx) => (
                    <div key={idx} className="flex items-center gap-4">
                      <span className="text-slate-800 font-semibold text-sm w-32 truncate shrink-0" title={dept.name}>
                        {dept.name}
                      </span>
                      <div className="rounded-full flex flex-1 h-3 overflow-hidden bg-slate-100 shadow-inner">
                        <div style={{ width: `${dept.notStartedPct}%` }} className="bg-slate-200 h-full transition-all duration-500" title={`Not Started: ${Math.round(dept.notStartedPct)}%`} />
                        <div style={{ width: `${dept.onTrackPct}%` }} className="bg-amber-400 h-full transition-all duration-500" title={`In Progress: ${Math.round(dept.onTrackPct)}%`} />
                        <div style={{ width: `${dept.completedPct}%` }} className="bg-emerald-500 h-full transition-all duration-500" title={`Completed: ${Math.round(dept.completedPct)}%`} />
                      </div>
                    </div>
                  ))
                )}
          ...
          <div className="flex flex-col gap-8 xl:w-80 shrink-0">
            ...
            <div className="rounded-xl bg-white shadow-sm border border-slate-200 flex flex-col p-6 gap-5 h-fit">
              <h2 className="font-bold text-lg text-slate-900 border-b border-slate-100 pb-3">Quick Actions</h2>
              
              <div className="flex flex-col gap-3">
                <label className="font-bold text-xs uppercase tracking-wider text-slate-500">Assign Shared Goal</label>
                <Link href="/admin/shared-goals" className="inline-flex font-semibold hover:bg-blue-700 transition-colors rounded-lg bg-blue-600 text-white text-sm px-4 py-2.5 justify-center items-center gap-2 w-full shadow-sm">
