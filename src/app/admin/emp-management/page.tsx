...
export default function EmployeeManagement() {
  ...
  async function loadData() {
    try {
      ...
      if (reportRes) {
        ...
        const employeeList = allProfiles.filter(
        ...
        const augmentedData = employeeList.map((emp: any) => {
          ...
          const empGoals = sheet
            ? goals.filter((g: any) => g.goal_sheet_id === sheet.id)
            : [];

          let totalProgress = 0;
          empGoals.forEach((g: any) => {
            totalProgress += g.progress || 0;
          });
          const avgProgress =
            empGoals.length > 0
              ? Math.round(totalProgress / empGoals.length)
              : 0;
  ...
  useEffect(() => {
    ...
    if (statusFilter !== "All Statuses") {
      if (statusFilter === "Submitted")
        result = result.filter((e) => e.sheet_status === "submitted");
      if (statusFilter === "Draft")
        result = result.filter((e) => e.sheet_status === "draft");
      if (statusFilter === "Not Started")
        result = result.filter((e) => e.sheet_status === "Not Started");
    }
  ...
  return (
    <div className="min-h-screen bg-[#f4f7fb] text-slate-900 flex w-full font-sans">
      <aside
        className={`shrink-0 min-h-screen bg-[#111115] text-white p-6 flex-col gap-8 w-64 fixed md:sticky top-0 z-50 transition-transform duration-300 ease-in-out ${mobileMenuOpen ? "translate-x-0 flex" : "-translate-x-full md:translate-x-0 md:flex hidden"}`}
      >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-sm flex items-center gap-5">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-3xl font-bold font-serif text-red-500">
                  {inactiveCount}
                </span>
              </div>
            </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
            <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-white">
              ...
              <span className="text-sm text-slate-500">
                Showing 1–{Math.min(8, filteredEmployees.length)} of{" "}
                  ...
                  {filteredEmployees.length === 0 ? (
                    ...
                    filteredEmployees.slice(0, 8).map((emp) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span
                            className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium border ${getDeptStyles(emp.department)}`}
                          >
                            {emp.department || "N/A"}
                          </span>
                        </td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span
                            className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium border capitalize ${getStatusStyles(emp.sheet_status)}`}
                          >
                            {emp.sheet_status}
                          </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center gap-3">
                            <div className="h-1.5 w-24 bg-slate-100 rounded-full overflow-hidden">
                              <div
                                className={`h-full rounded-full transition-all duration-500 ${emp.progress > 70 ? "bg-emerald-500" : emp.progress > 30 ? "bg-amber-400" : "bg-slate-300"}`}
                                style={{
                                  width: `${Math.max(0, Math.min(100, emp.progress))}%`,
                                }}
                              />
                            </div>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex flex-col text-sm text-slate-600">
                            <span>{formatDate(emp.last_checkin)}</span>
                          </div>
                        </td>
                      </tr>
                    ))
