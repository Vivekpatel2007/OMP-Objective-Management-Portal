...
export default function GoalSheetsAdministration() {
  ...
  async function loadData() {
    try {
      ...
      if (reportRes) {
        ...
        const leadershipList = allProfiles.filter(
          (p: any) => p.role === "manager" || p.role === "admin",
        );

        const enrichedSheets = sheets.map((sheet: any) => {
          const emp =
            allProfiles.find((p: any) => p.id === sheet.employee_id) || {};
          const manager =
            leadershipList.find((m: any) => m.id === emp.manager_id) || {};
          const sheetGoals = goals.filter(
            (g: any) => g.goal_sheet_id === sheet.id,
          );

          let totalProgress = 0;
          sheetGoals.forEach((g: any) => {
            totalProgress += g.progress || 0;
          });
          const avgProgress =
            sheetGoals.length > 0
              ? Math.round(totalProgress / sheetGoals.length)
              : 0;

          const lastUpdate = sheetGoals.reduce((latest: any, g: any) => {
            if (!g.updated_at) return latest;
            return !latest || new Date(g.updated_at) > new Date(latest)
              ? g.updated_at
              : latest;
  ...
  useEffect(() => {
    ...
    if (statusFilter !== "All Statuses") {
      ...
      if (statusFilter === "Not Started")
        result = result.filter((s) => s.status === "Not Started");
  ...
  const submittedCount = allSheets.filter(
    (s) => s.status === "submitted" || s.status === "approved",
  ).length;
  const inReviewCount = allSheets.filter(
    (s) => s.status === "submitted",
  ).length;
  const needsAttentionCount = allSheets.filter(
    (s) => s.status === "rejected" || s.status === "Not Started",
  ).length;

  const departments = [
    "All Departments",
    ...Array.from(new Set(allSheets.map((s) => s.department).filter(Boolean))),
  ];
  const managers = [
    "All Managers",
    ...Array.from(
      new Set(allSheets.map((s) => s.manager_name).filter(Boolean)),
    ),
  ];
      ...
      >
        ...
        <nav className="flex flex-col gap-1.5 mt-4">
          ...
          <Link
            ...
            className="rounded-lg bg-white/10 text-white text-sm font-medium flex px-4 py-3 items-center gap-3"
          >
      ...
      <main className="flex flex-col flex-1 min-w-0 overflow-auto">
        <div className="p-6 md:p-8 flex flex-col gap-6 max-w-7xl mx-auto w-full">
          <header className="flex flex-col md:flex-row justify-between md:items-start gap-4">
            <div className="flex items-start gap-3">
              <button
                ...
                className="md:hidden text-slate-500 mt-1"
              >
              ...
              <div className="flex flex-col gap-1">
                ...
                <p className="text-slate-500 text-sm">
                  View every employee goal sheet and inspect full details,
            ...
            <div className="flex items-center gap-3 w-full md:w-auto">
              <div className="relative w-full md:w-64">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-4 py-2 bg-[#1a1a1a] text-white placeholder-slate-400 border border-transparent rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-slate-500 transition-shadow"
                />
              </div>
            </div>
          </header>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="bg-white rounded-xl p-5 border border-slate-200 shadow-sm flex items-center gap-4">
              <div className="size-10 rounded-full bg-emerald-50 flex items-center justify-center text-emerald-500 shrink-0">
            ...
            <div className="bg-white rounded-xl p-5 border border-slate-200 shadow-sm flex items-center gap-4">
              <div className="size-10 rounded-full bg-rose-50 flex items-center justify-center text-rose-500 shrink-0">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    Browse sheets, status, and open the full record from the
                    details panel.
                  </p>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-3">
                <div className="relative">
                  <select
                    ...
                    className="appearance-none pl-4 pr-10 py-2.5 bg-white border border-slate-200 rounded-lg text-sm font-semibold text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500/20"
                  >
                    {departments.map((d) => (
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredSheets.length === 0 ? (
                    <tr>
                      ...
                      >
                        No goal sheets match your criteria.
                      </td>
                    </tr>
                  ) : (
                    filteredSheets.map((sheet) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span
                              className={`inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-semibold border ${getDeptStyles(sheet.department)}`}
                            >
                              {sheet.department}
                            </span>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex flex-col text-sm font-medium text-slate-700">
                              <span>{sheet.manager_name.split(" ")[0]}</span>
                              <span>
                                {sheet.manager_name
                                  .split(" ")
                                  .slice(1)
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span
                              className={`inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-semibold border capitalize ${getStatusStyles(sheet.status)}`}
                            >
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex items-center gap-3">
                              <div className="h-1.5 w-24 bg-slate-100 rounded-full overflow-hidden">
                                <div
                                  className={`h-full rounded-full transition-all duration-500 ${getProgressColor(sheet.progress)}`}
                                  style={{
                                    width: `${Math.max(0, Math.min(100, sheet.progress))}%`,
                                  }}
                                />
                              </div>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-center">
                            <button
                              ...
                              className="inline-flex items-center gap-1.5 text-blue-600 hover:text-blue-800 text-sm font-semibold transition-colors bg-blue-50 hover:bg-blue-100 px-3 py-1.5 rounded-lg"
                            >
            ...
            <div className="fixed inset-y-0 right-0 w-full md:w-[450px] bg-white shadow-2xl z-50 border-l border-slate-200 flex flex-col animate-in slide-in-from-right duration-300">
              <div className="p-6 border-b border-slate-100 flex items-start justify-between bg-slate-50/50">
                <div className="flex flex-col gap-1">
                  ...
                  <p className="text-sm text-slate-500">
                    Open objectives and active metrics.
                  </p>
                </div>
                <button
                  onClick={() => setSelectedSheet(null)}
              ...
              <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6">
                <div className="flex items-center gap-4 bg-white border border-slate-100 shadow-sm p-4 rounded-xl">
                  <div className="size-12 rounded-full font-bold text-lg flex items-center justify-center text-white bg-blue-600 shadow-sm">
                    {getInitials(selectedSheet.employee_name)}
                  </div>
                  <div className="flex flex-col">
                    <span className="font-bold text-slate-900">
                      {selectedSheet.employee_name}
                    </span>
                    <span className="text-sm text-slate-500">
                      {selectedSheet.employee_email}
                    </span>
                  </div>
                  <div className="ml-auto">
                    <span
                      className={`inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-semibold border capitalize ${getStatusStyles(selectedSheet.status)}`}
                    >
                ...
                <div className="flex flex-col gap-4">
                  <h4 className="font-bold text-slate-900">
                    Objectives ({selectedSheet.goals?.length || 0})
                  </h4>
                  {selectedSheet.goals?.length === 0 ? (
                    <div className="text-sm text-slate-500 py-4 text-center border border-dashed border-slate-200 rounded-lg">
                      No goals have been added to this sheet yet.
                    </div>
                  ) : (
                      ...
                      {selectedSheet.goals.map((goal: any) => (
                        ...
                        >
                          <div className="flex justify-between items-start mb-2">
                            ...
                            <span className="text-xs font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded border border-blue-100 shrink-0">
                              W: {goal.weightage}%
                            </span>
                          </div>
                          <div className="flex items-center justify-between text-xs text-slate-500 mb-2">
                            <span>
                              Target: {goal.target_value} {goal.uom_type}
                            </span>
                            <span>{goal.progress || 0}% Complete</span>
                          </div>
                          <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
                            <div
                              className={`h-full rounded-full transition-all duration-500 ${getProgressColor(goal.progress || 0)}`}
                              style={{
              ...
              <div className="p-5 border-t border-slate-100 bg-white">
                <button
                  onClick={() => setSelectedSheet(null)}
                  className="w-full py-2.5 bg-slate-900 text-white rounded-lg font-semibold text-sm hover:bg-slate-800 transition-colors"
                >
                  Close Details
                </button>
              </div>
