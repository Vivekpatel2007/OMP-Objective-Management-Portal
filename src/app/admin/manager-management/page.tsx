...
export default function ManagerManagement() {
  ...
  async function loadData() {
    try {
      ...
      if (reportRes) {
        ...
        const managersList = allProfiles.filter(
          (p: any) => p.role === "manager",
        );
        const employeesList = allProfiles.filter(
          (p: any) => p.role === "employee",
        );

        const augmentedData = managersList.map((mgr: any) => {
          const team = employeesList.filter(
            (emp: any) => emp.manager_id === mgr.id,
          );
          const teamIds = team.map((e) => e.id);
          const teamSheets = sheets.filter((s: any) =>
            teamIds.includes(s.employee_id),
          );
          const pendingApprovals = teamSheets.filter(
            (s: any) => s.submission_status === "submitted",
          ).length;
          const teamSheetIds = teamSheets.map((s) => s.id);
          const teamGoals = goals.filter((g: any) =>
            teamSheetIds.includes(g.goal_sheet_id),
          );
          ...
          const avgProgress =
            teamGoals.length > 0
              ? Math.round(totalProgress / teamGoals.length)
              : 0;

          const lastCheckin = teamGoals.reduce(
            (latest: any, g: any) => {
            ...
            teamSheets.reduce((latest: any, s: any) => {
              ...
              return !latest || new Date(s.updated_at) > new Date(latest)
                ? s.updated_at
                : latest;
            }, null),
          );
  ...
  const totalPendingApprovals = filteredManagers.reduce(
    (sum, m) => sum + m.pendingApprovals,
    0,
  );
  const avgTeamSize =
    totalManagers > 0
      ? (
          filteredManagers.reduce((sum, m) => sum + m.teamSize, 0) /
          totalManagers
        ).toFixed(1)
      : "0";
  const departments = [
    "All Departments",
    ...Array.from(new Set(managers.map((m) => m.department).filter(Boolean))),
  ];
            ...
            <div className="flex items-center justify-between p-5 border-b border-slate-100">
              <div>
                <h3 className="font-bold text-slate-900 text-lg">
                  {selectedManager.full_name}&apos;s Team
                </h3>
                <p className="text-sm text-slate-500">
                  {selectedManager.teamList?.length} members
                </p>
              </div>
              <button
            ...
            <div className="overflow-y-auto p-5 flex flex-col gap-3">
              {selectedManager.teamList?.length === 0 ? (
                ...
                selectedManager.teamList?.map((emp: any) => (
                  <div
                    key={emp.id}
                    className="flex items-center gap-3 p-3 rounded-lg border border-slate-100 hover:border-slate-200 bg-slate-50/50 transition-colors"
                  >
                    <div className="size-10 rounded-full bg-blue-100 text-blue-600 font-semibold text-sm flex items-center justify-center shrink-0">
                    ...
                    <span
                      className={`inline-flex items-center px-2 py-1 rounded-full text-[10px] font-medium border ${getDeptStyles(emp.department)}`}
                    >
                      {emp.department || "N/A"}
                    </span>
                  </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col gap-1">
                ...
                <p className="text-slate-500 text-sm">
                  Manage managers, their teams, and approval responsibilities
                </p>
              </div>
            </div>
          </header>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-sm flex items-center gap-5">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-3xl font-bold font-serif text-emerald-500">
                  {avgTeamSize}
                </span>
              </div>
            </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
            <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-white">
              ...
              <span className="text-sm text-slate-500">
                Showing 1–{Math.min(8, filteredManagers.length)} of{" "}
                    ...
                    filteredManagers.slice(0, 8).map((mgr) => (
                      ...
                      >
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center gap-3">
                            <div className="size-9 rounded-full bg-blue-600 text-white font-semibold text-xs flex items-center justify-center shadow-sm">
                              {getInitials(mgr.full_name)}
                            </div>
                            <div className="flex flex-col">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span
                            className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium border ${getDeptStyles(mgr.department)}`}
                          >
                            {mgr.department || "N/A"}
                          </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-center">
                          <div className="flex items-center justify-center gap-1.5 text-slate-600 font-medium text-sm">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-center">
                          <span
                            className={`inline-flex items-center justify-center size-6 rounded-full text-xs font-bold ${mgr.pendingApprovals > 0 ? "bg-amber-100 text-amber-700" : "bg-slate-100 text-slate-500"}`}
                          >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center gap-3">
                            <div className="h-1.5 w-24 bg-slate-100 rounded-full overflow-hidden">
                              <div
                                className={`h-full rounded-full transition-all duration-500 ${getProgressColor(mgr.avgProgress)}`}
                                style={{
                                  width: `${Math.max(0, Math.min(100, mgr.avgProgress))}%`,
                                }}
                              />
                            </div>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex flex-col text-sm text-slate-600">
                            <span>{formatDate(mgr.last_checkin)}</span>
                          </div>
                        </td>
            ...
            <div className="p-4 border-t border-slate-100 flex items-center justify-between bg-white">
              ...
              <div className="flex items-center gap-1 ml-auto sm:ml-0">
                ...
                <button className="size-8 rounded-md hover:bg-slate-50 text-slate-600 text-sm font-medium transition-colors">
                  2
                </button>
                <button className="size-8 rounded-md hover:bg-slate-50 text-slate-600 text-sm font-medium transition-colors">
                  3
