...
export default function EmployeeCheckinPage() {
  ...
  async function submitShared(sg: any) {
    ...
    if (
      !confirm(
        `Submit this shared goal update to ${ownerName}? You won't be able to edit it until it is reviewed.`,
      )
    )
      return;
  ...
  const handleSharedGoalUpdate = (
    assignmentId: string,
    field: string,
    value: number,
  ) => {
    setSharedGoals((prev) =>
      prev.map((g) => (g.id === assignmentId ? { ...g, [field]: value } : g)),
    );
  };

  const isSheetLocked =
    sheetStatus === "submitted" || sheetStatus === "approved";
  ...
  if (loading && !profile)
    return (
      <div className="h-screen bg-[#F8F9FC] flex items-center justify-center font-medium text-neutral-500">
        Loading Workspace...
      </div>
    );
  ...
  return (
    <div className="flex h-screen bg-[#F8F9FC] overflow-hidden">
      {/* SIDEBAR */}
      <aside
        className={`fixed md:static bg-[#0F1729] w-[230px] h-screen z-50 transition-transform flex flex-col ${mobile ? "translate-x-0" : "-translate-x-full"} md:translate-x-0`}
      >
        <div className="p-5">
          ...
          <div className="max-w-5xl mx-auto space-y-8">
            <div className="flex gap-6 border-b border-neutral-200">
              <button
                ...
                className={`pb-3 px-2 font-semibold text-sm border-b-2 transition-colors ${tab === "sheet" ? "border-indigo-600 text-indigo-600" : "border-transparent text-neutral-500 hover:text-neutral-800"}`}
              >
              ...
              <button
                ...
                className={`pb-3 px-2 font-semibold text-sm border-b-2 transition-colors ${tab === "shared" ? "border-indigo-600 text-indigo-600" : "border-transparent text-neutral-500 hover:text-neutral-800"}`}
              >
              ...
              <div className="flex bg-white rounded-xl p-1 shadow-sm border border-neutral-200">
                {["Q1", "Q2", "Q3", "Q4"].map((q) => (
                  <button
                    ...
                    className={`px-6 py-2 rounded-lg font-semibold text-sm ${quarter === q ? "bg-indigo-600 text-white" : "text-neutral-500 hover:bg-neutral-50"}`}
                  >
                    {q}
                  </button>
                ))}
              </div>
              {tab === "sheet" && isSheetLocked && (
                <span className="text-amber-600 font-semibold text-sm bg-amber-50 px-3 py-1.5 rounded-lg border border-amber-200 flex items-center gap-2">
                  <Lock size={14} /> Sheet is locked
                </span>
              )}
            </div>
                ...
                {goals.length === 0 ? (
                  <div className="bg-white rounded-2xl border border-dashed border-neutral-300 p-12 text-center flex flex-col items-center shadow-sm">
                    ...
                    <h3 className="text-lg font-semibold text-neutral-900">
                      No personal goals scheduled for {quarter}
                    </h3>
                    <p className="text-neutral-500 mt-1 text-sm max-w-sm">
                      There are no personal sheet goals assigned to you for this
                      specific quarter. Check another quarter or add goals to
                      your sheet.
                    </p>
                  </div>
                ) : (
                  goals.map((goal) => (
                    <div
                      key={`sheet-goal-${goal.id}`}
                      className={`bg-white rounded-2xl p-6 shadow-sm border border-neutral-200/60 ${isSheetLocked ? "opacity-90" : ""}`}
                    >
                      ...
                      <div className="grid lg:grid-cols-2 gap-6 bg-neutral-50/50 p-5 rounded-xl border border-neutral-100">
                        <div className="grid grid-cols-2 gap-4 text-sm">
                          <div>
                            <div className="text-neutral-500 text-xs uppercase tracking-wider font-semibold">
                              Target
                            </div>
                            <div className="font-semibold text-lg text-neutral-900 mt-1">
                              {goal.target_value}{" "}
                          ...
                          <div>
                            ...
                            <div className="font-semibold text-lg text-indigo-600 mt-1">
                              {goal.progress}%
                            </div>
                          </div>
                        </div>
              ...
              <div className="space-y-6">
                {displayedSharedGoals.length === 0 ? (
                  <div className="bg-white rounded-2xl border border-dashed border-neutral-300 p-12 text-center flex flex-col items-center shadow-sm">
                    ...
                    <h3 className="text-lg font-semibold text-neutral-900">
                      No Shared Goals for {quarter}
                    </h3>
                    <p className="text-neutral-500 mt-1 text-sm max-w-sm">
                      There are currently no shared goals assigned to you
                      targeting this specific quarter. Check another quarter to
                  ...
                  displayedSharedGoals.map((sg) => {
                    ...
                    return (
                      <div
                        key={`shared-goal-${sg.id}`}
                        className={`bg-white rounded-2xl p-6 shadow-sm border border-neutral-200/60 transition-all hover:shadow-md ${isSgLocked ? "opacity-90" : ""}`}
                      >
                        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-4">
                          ...
                          <div className="flex flex-col items-end gap-2 shrink-0">
                            <div className="flex gap-2">
                              <Badge
                                variant="secondary"
                                className="bg-indigo-50 text-indigo-700 capitalize border-indigo-100"
                              >
                                {sg.shared_goals?.assignment_type || "Custom"}{" "}
                                Assignment
                              </Badge>
                              {sg.shared_goals?.department && (
                                <Badge
                                  variant="outline"
                                  className="text-neutral-600 bg-neutral-50 border-neutral-200 flex items-center gap-1"
                                >
                                  <Briefcase size={12} />{" "}
                                  {sg.shared_goals?.department}
                                </Badge>
                        ...
                        <div className="grid lg:grid-cols-2 gap-6 bg-indigo-50/30 p-5 rounded-xl border border-indigo-50 mt-6">
                          <div className="grid grid-cols-2 gap-4 text-sm items-center">
                            <div>
                              ...
                              <div className="font-semibold text-lg text-neutral-900 mt-1">
                                {sg.shared_goals?.target_value}{" "}
                                {sg.shared_goals?.uom_type}
                              </div>
                            </div>
                          </div>
                            ...
                            {!isSgLocked && (
                              <div className="flex gap-2">
                                ...
                                <button
                                  ...
                                  disabled={
                                    savingId === "submit-" + sg.id || isSgLocked
                                  }
...
function Nav({ icon: Icon, label, href, active }: any) {
  return (
    <Link
      href={href}
      className={`flex gap-3 px-4 py-3 rounded-xl transition-colors ${active ? "bg-indigo-600 text-white font-medium" : "text-white/70 hover:bg-white/10"}`}
    >
      <Icon size={18} /> {label}
    </Link>
  );
