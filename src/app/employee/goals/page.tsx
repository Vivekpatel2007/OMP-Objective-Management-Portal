...
export default function GoalSheet() {
  ...
  async function load() {
    try {
      ...
      if (activeCycle) {
        ...
        else if (isWithin(activeCycle.q3_start, activeCycle.q3_end)) currentQ = "Q3";
        else if (isWithin(activeCycle.q4_start, activeCycle.q4_end)) currentQ = "Q4";
      }
  ...
  const currentGoals = goals.filter((g) => (g.quarter || 'Q1') === activeQuarter);
  ...
  const pastGoals = goals.filter((g) => {
    ...
    return qOrder[goalQ as keyof typeof qOrder] < qOrder[activeQuarter as keyof typeof qOrder];
  });

  // Calculate weightage specifically for the active quarter so it resets to 0% initially
  const totalWeight = currentGoals.reduce((a, b) => a + Number(b.weightage || 0), 0);
  ...
  const renderGoalsList = (list: Goal[], isReadOnly: boolean) => {
      ...
      return (
        <Card className="rounded-2xl border-dashed border-2 bg-transparent shadow-none">
          <CardContent className="p-10 text-center text-neutral-500">
            <div className="font-medium mb-2 text-lg text-neutral-800">Your {activeQuarter} Sheet is Empty</div>
            <p className="text-sm">Start adding your goals for this quarter. Weightage must total 100%.</p>
            {!isReadOnly && !isSheetLocked && (
              <Link href="/employee/goals/create">
                <Button className="mt-5 bg-indigo-600 hover:bg-indigo-700 text-white"><Plus size={16} className="mr-2" /> Create First Goal</Button>
              </Link>
            )}
          </CardContent>
        </Card>
    ...
    return list.map((goal) => 
      editingGoalId === goal.id ? (
        ...
        <Card key={goal.id} className={`p-5 rounded-2xl shadow-sm border-neutral-200/60 ${isReadOnly ? "bg-slate-50" : ""}`}>
            ...
            <div className="flex-1">
              <div className="flex justify-between items-start">
                ...
                <div className="flex flex-col items-end gap-2">
                  {isReadOnly && <Badge variant="outline" className="text-slate-500">{goal.quarter}</Badge>}
                  <Badge variant="secondary" className="bg-indigo-50 text-indigo-700">{goal.progress || 0}% Progress</Badge>
                </div>
              </div>
              <div className={`grid md:grid-cols-4 gap-5 mt-6 p-4 rounded-xl border border-neutral-100 ${isReadOnly ? "bg-white" : "bg-neutral-50"}`}>
                <div><div className="text-xs text-neutral-500 mb-1.5 uppercase font-medium tracking-wider">Thrust Area</div><Badge variant="outline" className="bg-white border-neutral-200">{goal.thrust_area || "N/A"}</Badge></div>
                <div><div className="text-xs text-neutral-500 mb-1.5 uppercase font-medium tracking-wider">UOM</div><div className="text-sm text-neutral-700 truncate font-medium">{goal.uom_type || "-"}</div></div>
                ...
                <div><div className="text-xs text-neutral-500 mb-1.5 uppercase font-medium tracking-wider">Weight</div><div className="text-sm font-medium text-indigo-600">{goal.weightage}%</div></div>
              </div>
  ...
  if (loading) return <div className="h-screen grid place-items-center bg-[#F8F9FC]">Loading...</div>;
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        <header className="bg-white px-5 py-4 border-b flex justify-between items-center z-10">
          ...
          <div className="flex gap-4 items-center">
            <div className="text-sm font-medium text-neutral-700">{profile?.full_name}</div>
          </div>
        </header>
          ...
          <div className="max-w-5xl mx-auto">
            ...
            {!hasGoalSheet && (
              <div className="mb-8 bg-white border border-indigo-100 rounded-2xl p-8 text-center flex flex-col items-center justify-center shadow-sm">
                ...
                <h3 className="text-xl font-bold text-neutral-900 mb-2">No Goal Sheet for {activeQuarter}</h3>
                <p className="text-neutral-500 mb-6 max-w-sm text-sm">
                  You need to generate a new goal sheet for the active quarter before adding goals.
                </p>
                <Button 
                  onClick={handleCreateSheet}
                  className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2.5 rounded-lg shadow-sm shadow-indigo-600/20"
                >
                  <Plus size={18} className="mr-2" /> Create {activeQuarter} Goal Sheet
                </Button>
              </div>
            )}
              ...
              <>
                <div className="flex justify-between flex-wrap gap-4 items-end mb-8">
                  <div>
                    ...
                    <p className="text-neutral-500 mt-1 text-sm">Manage your goals for the current active cycle.</p>
                  ...
                  {!isSheetLocked && currentGoals.length > 0 && (
                    <div className="flex gap-2">
                      ...
                      <Button disabled={remaining !== 0} onClick={submit} className="bg-indigo-600 hover:bg-indigo-700">
                        <Send size={16} className="mr-2" /> Submit {activeQuarter} Sheet
                      </Button>
                    </div>
                  )}
                </div>
                ...
                {viewTab === "current" ? (
                  <div className="space-y-6">
                    <Card className="rounded-2xl shadow-sm border-neutral-200/60">
                      <CardHeader className="pb-3"><CardTitle className="text-base">{activeQuarter} Weightage Summary</CardTitle></CardHeader>
                      <CardContent>
